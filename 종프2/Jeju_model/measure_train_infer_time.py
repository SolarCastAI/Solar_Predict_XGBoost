...
class LSTM_Pattern(nn.Module):
    ...
    def predict(self, test_loader, measure_time=True):
                """
                모델 예측 함수
                """
                self.eval()
                ...
                inference_start = time.time() if measure_time else None
                ...
                inference_time = time.time() - inference_start if measure_time else None
                
                if measure_time:
                    return np.array(predictions), np.array(actuals), inference_time
                else:
...
def predict(self, test_loader, measure_time=False):
            """
            모델 예측 함수
            """
            self.eval()
...
if __name__ == "__main__":
    try:
        ...
        time_results['LSTM_train_time'] = lstm_train_time
        
        print(f"\n⏱️ LSTM 학습 시간: {lstm_train_time:.2f}초 ({lstm_train_time/60:.2f}분)")
        ...
        time_results['GRU_train_time'] = gru_train_time
        
        print(f"\n⏱️ GRU 학습 시간: {gru_train_time:.2f}초 ({gru_train_time/60:.2f}분)")
        ...
        # LSTM 예측 및 평가 (추론 시간 측정)
        lstm_predictions_scaled, lstm_actuals_scaled, lstm_inference_time = lstm_model.predict(
            test_loader, measure_time=True
        )
        time_results['LSTM_inference_time'] = lstm_inference_time
        ...
        print(f"⏱️ 추론 시간: {lstm_inference_time:.4f}초")
        
        # GRU 예측 및 평가 (추론 시간 측정)
        gru_predictions_scaled, gru_actuals_scaled, gru_inference_time = gru_model.predict(
            test_loader, measure_time=True
        )
        time_results['GRU_inference_time'] = gru_inference_time
        ...
        print(f"⏱️ 추론 시간: {gru_inference_time:.4f}초")
        ...
        # Train set 예측값 생성
        lstm_preds_train_scaled, _ = lstm_model.predict(train_loader_stack, measure_time=False)
        gru_preds_train_scaled, _ = gru_model.predict(train_loader_stack, measure_time=False)
        
        # Validation set 예측값 생성
        lstm_preds_val_scaled, _ = lstm_model.predict(val_loader_stack, measure_time=False)
        gru_preds_val_scaled, _ = gru_model.predict(val_loader_stack, measure_time=False)
        ...
        time_results['XGBoost_train_time'] = xgb_train_time
        
        print(f"\n⏱️ XGBoost 학습 시간: {xgb_train_time:.2f}초 ({xgb_train_time/60:.2f}분)")
        
        # 스태킹 모델 예측 및 평가 (추론 시간 측정)
        xgb_inference_start = time.time()
        stacked_pred_test = xgb_stacking_regressor.predict(X_test_stack)
        xgb_inference_time = time.time() - xgb_inference_start
        time_results['XGBoost_inference_time'] = xgb_inference_time
        ...
        print(f"⏱️ 추론 시간: {xgb_inference_time:.4f}초")
        ...
        print(f"{'='*80}")
        print(f"\n{'[학습 시간]':<30}")
        print(f"{'LSTM 학습 시간:':<30} {time_results['LSTM_train_time']:.2f}초 ({time_results['LSTM_train_time']/60:.2f}분)")
        print(f"{'GRU 학습 시간:':<30} {time_results['GRU_train_time']:.2f}초 ({time_results['GRU_train_time']/60:.2f}분)")
        print(f"{'XGBoost 학습 시간:':<30} {time_results['XGBoost_train_time']:.2f}초 ({time_results['XGBoost_train_time']/60:.2f}분)")
        print(f"{'총 학습 시간:':<30} {sum([time_results['LSTM_train_time'], time_results['GRU_train_time'], time_results['XGBoost_train_time']]):.2f}초")
        
        print(f"\n{'[추론 시간]':<30}")
        print(f"{'LSTM 추론 시간:':<30} {time_results['LSTM_inference_time']:.4f}초")
        print(f"{'GRU 추론 시간:':<30} {time_results['GRU_inference_time']:.4f}초")
        print(f"{'XGBoost 추론 시간:':<30} {time_results['XGBoost_inference_time']:.4f}초")
        
        # 테스트 샘플당 추론 시간 계산
        n_test_samples = len(y_test_stack)
        print(f"\n{'[샘플당 추론 시간]':<30} (테스트 샘플 수: {n_test_samples})")
        print(f"{'LSTM:':<30} {time_results['LSTM_inference_time']/n_test_samples*1000:.4f}ms/sample")
        print(f"{'GRU:':<30} {time_results['GRU_inference_time']/n_test_samples*1000:.4f}ms/sample")
        print(f"{'XGBoost:':<30} {time_results['XGBoost_inference_time']/n_test_samples*1000:.4f}ms/sample")
