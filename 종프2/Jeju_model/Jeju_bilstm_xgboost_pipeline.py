...
class LSTM_Pattern(nn.Module):
    """
    Bidirectional LSTM + Bi-Attention 모델 (패턴 임베딩 포함)
    - BiLSTM을 사용하여 앞/뒤 문맥을 모두 반영
    - Bi-Attention: LSTM 출력 간 상호 attention을 계산하여 context 벡터 생성
    - Residual + LayerNorm 적용
    """
    def __init__(self, weather_feature_dim=4, pattern_embedding_dim=8, hidden_dim=64, num_layers=2, n_patterns=5, dropout=0.3):
        ...
        # Bi-Attention parameters
        self.proj_dim = self.lstm_hidden_size * self.num_directions
        self.scale = math.sqrt(self.proj_dim)

        # After bi-attention we'll concat several tensors -> project back to a fixed dim
        # concat: [H, Context1, H * Context1, H * Context2] -> 4 * proj_dim
        self.post_attn_proj = nn.Linear(4 * self.proj_dim, self.proj_dim)

        # Residual / Norm / Dropout
        self.layer_norm = nn.LayerNorm(self.proj_dim)
        self.dropout = nn.Dropout(dropout)

        # Pooling projection (last_step + mean_pool = 2 * proj_dim -> proj_dim)
        self.pooling_proj = nn.Linear(2 * self.proj_dim, self.proj_dim)

        # Fully connected head
        self.fc1 = nn.Linear(self.proj_dim, 128)
        self.bn1 = nn.BatchNorm1d(128)
        self.fc2 = nn.Linear(128, 64)
        self.bn2 = nn.BatchNorm1d(64)
        self.fc3 = nn.Linear(64, 1)

    def bi_attention(self, H):
        """
        H: (batch, seq_len, proj_dim)  # proj_dim = hidden_dim * num_directions
        returns: enhanced representation (batch, seq_len, proj_dim)
        """
        batch_size, seq_len, _ = H.shape
        
        # similarity matrix S: (B, T, T)
        # scaled dot-product
        # S_ij = (H_i · H_j) / scale
        S = torch.bmm(H, H.transpose(1, 2)) / (self.scale + 1e-9)  # (B, T, T)

        # Context from row-wise attention (attn over time for each query position)
        A_row = F.softmax(S, dim=-1)  # sum over j
        C_row = torch.bmm(A_row, H)   # (B, T, proj_dim)

        # Context from column-wise attention (attn over queries for each key position)
        A_col = F.softmax(S, dim=1)   # sum over i
        C_col = torch.bmm(A_col.transpose(1, 2), H)  # (B, T, proj_dim)

        # Combine: [H, C_row, H * C_row, H * C_col]
        H_mul_Crow = H * C_row
        H_mul_Ccol = H * C_col

        concat = torch.cat([H, C_row, H_mul_Crow, H_mul_Ccol], dim=-1)  # (B, T, 4*proj_dim)
        projected = self.post_attn_proj(concat)  # (B, T, proj_dim)

        # Residual + LayerNorm
        out = self.layer_norm(projected + H)
        out = self.dropout(out)
        return out

    def forward(self, x):
        """
        x: (batch, seq_len, total_features) where total_features = weather(4) + 1(pattern)
        """
        batch_size, seq_len, total_features = x.shape

        # split weather and pattern
        weather_data = x[:, :, :4]  # (B, T, 4)
        pattern_data = x[:, :, 4].long()  # (B, T)
        
        # 패턴 데이터 범위 체크 및 클램핑
        pattern_data = torch.clamp(pattern_data, 0, self.n_patterns - 1)

        # pattern embedding
        pattern_emb = self.pattern_embed(pattern_data)  # (B, T, embed_dim)

        # combined features
        combined = torch.cat([weather_data, pattern_emb], dim=-1)  # (B, T, lstm_input_size)

        # LSTM
        lstm_out, (h_n, c_n) = self.lstm(combined)  # lstm_out: (B, T, hidden_dim * num_directions)

        # Bi-Attention on LSTM outputs
        attn_out = self.bi_attention(lstm_out)  # (B, T, proj_dim)

        # Pooling: combine last timestep and mean to capture both recency and global context
        last_step = attn_out[:, -1, :]  # (B, proj_dim)
        mean_pool = attn_out.mean(dim=1)  # (B, proj_dim)
        pooled = torch.cat([last_step, mean_pool], dim=-1)  # (B, 2*proj_dim)

        # Project pooled back to proj_dim for FC head
        pooled = F.relu(self.pooling_proj(pooled))  # (B, proj_dim)
    ...
    def train_model(self, train_loader, val_loader=None, epochs=10, lr=0.001):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx, (batch_X, batch_y) in enumerate(train_loader):
                try:
                    ...
                    if epoch == 0 and batch_idx == 0:
                        print(f"First batch - Input shape: {batch_X.shape}, Output shape: {preds.shape}")
                        print(f"Pattern data range: {batch_X[:, :, 4].min().item():.0f} to {batch_X[:, :, 4].max().item():.0f}")
            ...
            if train_batches == 0:
                print("No training batches processed successfully!")
                break
