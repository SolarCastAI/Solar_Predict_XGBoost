...
from sklearn.linear_model import Ridge  # Ridge 회귀 추가
...
class RidgeRegressor:
    ...
    def __init__(self, alpha=1.0, normalize=True):
        self.ridge = Ridge(alpha=alpha, normalize=normalize)
        self.scaler = StandardScaler()
        self.feature_scaler = StandardScaler()
        self.is_fitted = False
        
    def train_with_cv(self, X_train, y_train, X_val=None, y_val=None, alphas=None):
        """
        교차검증을 통한 최적 alpha 값 찾기 및 학습
        """
        if alphas is None:
            alphas = [0.1, 0.5, 1.0, 5.0, 10.0, 50.0, 100.0, 500.0, 1000.0]
        
        print("릿지 회귀 최적 하이퍼파라미터 탐색 중...")
        
        # 특성 스케일링
        X_train_scaled = self.feature_scaler.fit_transform(X_train)
        
        # GridSearchCV를 사용한 최적 alpha 탐색
        ridge_cv = GridSearchCV(
            Ridge(normalize=False),  # 이미 스케일링 했으므로 normalize=False
        ...
        self.ridge = ridge_cv.best_estimator_
        self.is_fitted = True
        
        print(f"최적 alpha: {ridge_cv.best_params_['alpha']}")
        print(f"교차검증 MAE: {-ridge_cv.best_score_:.4f}")
        ...
        if X_val is not None and y_val is not None:
            ...
            print(f"검증 데이터 성능 - MAE: {val_mae:.4f}, RMSE: {val_rmse:.4f}, MAPE: {val_mape:.2f}%")
            
        return ridge_cv.best_params_['alpha']
    ...
    def predict(self, X):
        ...
        if not self.is_fitted:
            raise ValueError("모델이 학습되지 않았습니다. train_with_cv를 먼저 실행하세요.")
...
class LSTM_Pattern_Ridge(nn.Module):
    """
    릿지 정규화를 적용한 LSTM 패턴 모델 (과적합 방지 강화)
    """
    def __init__(self, weather_feature_dim=4, pattern_embedding_dim=8, hidden_dim=64, 
                 num_layers=2, n_patterns=5, ridge_lambda=0.01):
        super(LSTM_Pattern_Ridge, self).__init__()
        ...
        self.lstm = nn.LSTM(input_size=lstm_input_size, hidden_size=hidden_dim, 
                              num_layers=num_layers, batch_first=True, dropout=0.3)  # dropout 증가
        
        # Attention mechanism
        self.attention = nn.MultiheadAttention(embed_dim=hidden_dim, num_heads=4, 
                                             batch_first=True, dropout=0.2)
        
        # FC layers with increased regularization
        self.fc1 = nn.Linear(hidden_dim, 128)
        self.fc2 = nn.Linear(128, 64)
        self.fc3 = nn.Linear(64, 1)
        self.dropout = nn.Dropout(0.4)  # dropout 증가
    ...
    def train_model(self, train_loader, val_loader=None, epochs=15, lr=0.001):
        """
        릿지 정규화를 적용한 모델 학습 함수
        """
        ...
        print(f"총 {epochs} 에포크 학습을 시작합니다 (릿지 정규화 적용)...")
        ...
        for epoch in range(epochs):
            ...
            for batch_X, batch_y in train_loader:
                ...
                # MSE Loss + L2 Regularization (Ridge)
                mse_loss = criterion(preds, batch_y)
                l2_reg = self.l2_regularization()
                total_loss = mse_loss + self.ridge_lambda * l2_reg
...
def xgb_stacking_model(X_train, y_train, X_val, y_val, X_test, y_test, plotting=False):
    """
    LSTM & GRU 예측을 포함한 스태킹 XGBoost 모델 (릿지 정규화 포함)
    """
    # XGBoost 모델 정의 (릿지 효과를 위한 정규화 강화)
    xgb_regressor = xgb.XGBRegressor(
        ...
        reg_lambda=0.3,  # L2 정규화 강화 (릿지 효과)
        eval_metric='mae'
    )
    
    print("XGBoost 스태킹 모델 학습 중 (릿지 정규화 적용)...")
    # 모델 학습
    xgb_regressor.fit(
        X_train, 
        y_train, 
...
if __name__ == "__main__":
    try:
        ...
        # 5. Ridge 회귀 모델 학습 (베이스라인)
        print("\n" + "="*60)
        print("Ridge 회귀 모델 학습 시작...")
        print("="*60)
        
        ridge_model = RidgeRegressor()
        
        # 시퀀스 생성으로 줄어든 크기에 맞춰 Ridge용 데이터 준비
        X_ridge_train = np.hstack([features_train[seq_len:], pattern_train[seq_len:]])
        y_ridge_train = targets_train[seq_len:]
        X_ridge_val = np.hstack([features_val[seq_len:], pattern_val[seq_len:]])
        y_ridge_val = targets_val[seq_len:]
        X_ridge_test = np.hstack([features_test[seq_len:], pattern_test[seq_len:]])
        y_ridge_test = targets_test[seq_len:]
        
        # Ridge 모델 학습 및 평가
        best_alpha = ridge_model.train_with_cv(X_ridge_train, y_ridge_train, 
                                              X_ridge_val, y_ridge_val)
        
        ridge_pred_test = ridge_model.predict(X_ridge_test)
        mae_ridge = mean_absolute_error(y_ridge_test, ridge_pred_test)
        rmse_ridge = calculate_rmse(y_ridge_test, ridge_pred_test)
        mape_ridge = calculate_mape(y_ridge_test, ridge_pred_test)
        ...
        # 6. LSTM 모델 생성 및 학습 (릿지 정규화 적용)
        print("\n" + "="*60)
        print("LSTM 모델 학습 시작 (릿지 정규화 적용)...")
        print("="*60)
        
        lstm_model = LSTM_Pattern_Ridge(weather_feature_dim=4, pattern_embedding_dim=8, 
        ...
        # 7. GRU 모델 생성 및 학습 (릿지 정규화 적용)
        print("\n" + "="*60)
        print("GRU 모델 학습 시작 (릿지 정규화 적용)...")
        print("="*60)
        
        gru_model = GRU_Pattern_Ridge(weather_feature_dim=4, pattern_embedding_dim=8, 
                                     hidden_dim=128, num_layers=2, n_patterns=5,
                                     ridge_lambda=0.01)
        ...
        ridge_preds_train = ridge_model.predict(X_ridge_train)
        ...
        ridge_preds_val = ridge_model.predict(X_ridge_val)
        ...
        ridge_preds_test = ridge_pred_test.reshape(-1, 1)
        
        # 10. XGBoost 스태킹 학습을 위한 데이터 준비
        ...
        X_train_stack = np.hstack([
            ...
            ridge_preds_train.reshape(-1, 1)   # Ridge 예측값 (1개)
        ])
        ...
        print(f"스태킹 특성 차원: {X_train_stack.shape[1]} (기상 4 + 패턴 8 + LSTM 1 + GRU 1 + Ridge 1)")
        
        # 11. XGBoost 스태킹 모델 학습 및 평가
        print("\n" + "="*60)
        print("XGBoost 스태킹 앙상블 모델 학습 시작...")
        print("="*60)
        
        mae_stacked, rmse_stacked, mape_stacked, xgb_stacking_regressor = xgb_stacking_model(
            X_train_stack, y_train_stack, X_val_stack, y_val_stack, X_test_stack, y_test_stack
        )
        
        # 12. 최종 성능 비교
        print(f"\n{'='*80}")
        print("최종 모델 성능 비교 (릿지 정규화 적용)")
        print(f"{'='*80}")
        print(f"{'모델':<25} {'MAE':<10} {'RMSE':<10} {'MAPE (%)':<10}")
        print("-" * 60)
        print(f"{'Ridge Regression':<25} {mae_ridge:<10.4f} {rmse_ridge:<10.4f} {mape_ridge:<10.2f}")
        print(f"{'LSTM (Ridge)':<25} {mae_lstm:<10.4f} {rmse_lstm:<10.4f} {mape_lstm:<10.2f}")
        print(f"{'GRU (Ridge)':<25} {mae_gru:<10.4f} {rmse_gru:<10.4f} {mape_gru:<10.2f}")
        print(f"{'XGBoost Stacking':<25} {mae_stacked:<10.4f} {rmse_stacked:<10.4f} {mape_stacked:<10.2f}")
        
        # 성능 개선율 계산
        best_individual = min(mae_ridge, mae_lstm, mae_gru)
        ...
        ridge_importance = ridge_model.get_feature_importance([
            '기온', '강수량', '일조', '일사량', '패턴', '연간_sin', '연간_cos', 
            '일간_sin', '일간_cos', '기온×일사량', '일조×일사량', '무강수여부'
        ])
        print(f"\n=== Ridge 회귀 특성 중요도 (계수 절댓값) ===")
        for feature, importance in sorted(ridge_importance.items(), key=lambda x: x[1], reverse=True):
            print(f"{feature}: {importance:.4f}")
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title('LSTM Learning Curves (Ridge)')
        plt.legend()
        ...
        plt.xlabel('Epoch')
        plt.ylabel('Loss')
        plt.title('GRU Learning Curves (Ridge)')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        plt.subplot(4, 4, 3)
        plt.plot(y_ridge_test[:test_range], label='Actual', alpha=0.8, linewidth=2)
        plt.plot(ridge_pred_test[:test_range], label='Ridge', alpha=0.7)
        plt.xlabel('Time')
        plt.ylabel('발전량 (MWh)')
        plt.title(f'Ridge 예측 결과\nMAE: {mae_ridge:.3f}')
        plt.legend()
        plt.grid(True, alpha=0.3)
        ...
        # 3. 산점도 (실제값 vs 예측값)
        plt.subplot(4, 4, 7)
        plt.scatter(y_ridge_test, ridge_pred_test, alpha=0.5)
        plt.plot([y_ridge_test.min(), y_ridge_test.max()], 
                [y_ridge_test.min(), y_ridge_test.max()], 'r--', lw=2)
        plt.xlabel('실제값')
        plt.ylabel('예측값')
        ...
        # 4. 특성 중요도 비교
        plt.subplot(4, 4, 11)
        ridge_features = list(ridge_importance.keys())
        ridge_values = list(ridge_importance.values())
        
        # 상위 8개 특성만 표시
        top_ridge = sorted(zip(ridge_features, ridge_values), key=lambda x: x[1], reverse=True)[:8]
        features, values = zip(*top_ridge)
        
        plt.barh(features, values)
        plt.xlabel('중요도 (계수 절댓값)')
        plt.title('Ridge 회귀 특성 중요도 (Top 8)')
        plt.grid(True, alpha=0.3)
        ...
        }).sort_values('importance', ascending=True).tail(10)  # 상위 10개
        
        plt.barh(importance_df['feature'], importance_df['importance'])
        plt.xlabel('중요도')
        plt.title('XGBoost 스태킹 특성 중요도 (Top 10)')
        plt.grid(True, alpha=0.3)
        
        # 5. 잔차 분석
        ridge_residuals = y_ridge_test - ridge_pred_test
        ...
        plt.subplot(4, 4, 13)
        plt.scatter(ridge_pred_test, ridge_residuals, alpha=0.5)
        ...
        models = ['Ridge\nRegression', 'LSTM\n(Ridge)', 'GRU\n(Ridge)', 'XGBoost\nStacking']
        mae_scores = [mae_ridge, mae_lstm, mae_gru, mae_stacked]
        rmse_scores = [rmse_ridge, rmse_lstm, rmse_gru, rmse_stacked]
        mape_scores = [mape_ridge, mape_lstm, mape_gru, mape_stacked]
        
        x = np.arange(len(models))
        width = 0.25
        
        plt.subplot(1, 3, 1)
        bars = plt.bar(x, mae_scores, width, alpha=0.8, 
                      color=['skyblue', 'lightgreen', 'lightcoral', 'gold'])
        plt.xlabel('모델')
        plt.ylabel('MAE')
        plt.title('MAE 비교 (릿지 정규화 적용)')
        plt.xticks(x, models)
        plt.grid(True, alpha=0.3)
        ...
        plt.subplot(1, 3, 2)
        bars = plt.bar(x, rmse_scores, width, alpha=0.8, 
                      color=['skyblue', 'lightgreen', 'lightcoral', 'gold'])
        plt.xlabel('모델')
        plt.ylabel('RMSE')
        plt.title('RMSE 비교 (릿지 정규화 적용)')
        plt.xticks(x, models)
        plt.grid(True, alpha=0.3)
        ...
        plt.subplot(1, 3, 3)
        bars = plt.bar(x, mape_scores, width, alpha=0.8, 
                      color=['skyblue', 'lightgreen', 'lightcoral', 'gold'])
        plt.xlabel('모델')
        plt.ylabel('MAPE (%)')
        plt.title('MAPE 비교 (릿지 정규화 적용)')
        plt.xticks(x, models)
        plt.grid(True, alpha=0.3)
        
        # 값 표시
        for i, (bar, score) in enumerate(zip(bars, mape_scores)):
        ...
        print("과적합 방지 효과 분석")
        print(f"{'='*80}")
        print("릿지 정규화를 통한 개선사항:")
        print("1. L2 정규화를 통한 모델 파라미터 제약")
        print("2. 향상된 드롭아웃 비율 (0.3 → 0.4)")
        print("3. 강화된 그래디언트 클리핑 (1.0 → 0.5)")
        print("4. 조기 종료 patience 감소 (10 → 8)")
        print("5. XGBoost 정규화 파라미터 강화")
        print("6. Ridge 회귀 베이스라인 추가")
        
        # 모델 복잡도 vs 성능 분석
        complexity_scores = [1, 4, 4, 5]  # 모델 복잡도 점수 (1: 가장 단순, 5: 가장 복잡)
        
        plt.figure(figsize=(10, 6))
        plt.scatter(complexity_scores, mae_scores, s=200, alpha=0.7, 
                   c=['skyblue', 'lightgreen', 'lightcoral', 'gold'])
        
        for i, model in enumerate(models):
            plt.annotate(model.replace('\n', ' '), 
                        (complexity_scores[i], mae_scores[i]),
                        xytext=(5, 5), textcoords='offset points', fontsize=10)
        
        plt.xlabel('모델 복잡도')
        plt.ylabel('MAE')
        plt.title('모델 복잡도 vs 성능 (과적합 방지 효과)')
        plt.grid(True, alpha=0.3)
        plt.show()
        
        print(f"\n{'='*80}")
        print("모델 학습 및 평가 완료! (릿지 정규화 적용)")
        print(f"{'='*80}")
        print("주요 개선사항:")
        print("✓ Ridge 회귀 베이스라인 모델 추가")
        print("✓ LSTM/GRU 모델에 L2 정규화 적용")
        print("✓ 강화된 드롭아웃 및 정규화 기법")
        print("✓ XGBoost 스태킹에 릿지 정규화 강화")
        print("✓ 교차검증 기반 하이퍼파라미터 최적화")
        
        # 최종 추천 모델
        best_model_idx = mae_scores.index(min(mae_scores))
        best_model_name = models[best_model_idx]
        print(f"\n🏆 최고 성능 모델: {best_model_name.replace(chr(10), ' ')}")
        print(f"   MAE: {mae_scores[best_model_idx]:.4f}")
        print(f"   RMSE: {rmse_scores[best_model_idx]:.4f}")
        print(f"   MAPE: {mape_scores[best_model_idx]:.2f}%")
    ...
    def train_model(self, train_loader, val_loader=None, epochs=15, lr=0.001):
        ...
        """
        criterion = nn.MSELoss()
        optimizer = optim.AdamW(self.parameters(), lr=lr, weight_decay=1e-3)  # weight_decay 증가
        scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=epochs)
        ...
        best_val_loss = float('inf')
        patience = 8  # patience 감소로 조기 종료 강화
        patience_counter = 0
...
class GRU_Pattern_Ridge(nn.Module):
    """
    릿지 정규화를 적용한 GRU 패턴 모델
    """
    def __init__(self, weather_feature_dim=4, pattern_embedding_dim=8, hidden_dim=64, 
                 num_layers=2, n_patterns=5, ridge_lambda=0.01):
        super(GRU_Pattern_Ridge, self).__init__()
